...
- 5つの観点（呼び出し方式/入出力契約/信頼性・コスト/安全性UX/複数LLM呼び出しの組み合わせ）を漏れなく検討できる
- `app/`の設計判断と異なる選択をする場合、その理由を説明できる

## 概要

本教材はチュートリアル全体の最終演習です。架空の新機能を題材に、
01〜05の観点でミニ設計書を書きます。実装コードは書きません。
...
演習で使うテンプレートは次の5見出しです。01〜05章で学んだ観点に
そのまま対応しています。
...
- [ ] 5つの観点（呼び出し方式/入出力契約/信頼性・コスト/安全性UX/
      複数LLM呼び出しの組み合わせ）を漏れなく検討できる
