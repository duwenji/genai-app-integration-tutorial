...
- `app/`内の生成AI活用箇所（10箇所）を、01〜04で学んだ観点でマッピングした
  一覧表を読み解ける
...
- **04章（事実/考察分離・免責事項）**: 全10箇所が、Python側で計算した事実データを
...
flowchart TB
    subgraph cat01["01. Invocation & Architecture"]
        c01n["全10箇所共通: llm_client.call_llm()（CLIサブプロセス方式）"]
    end
